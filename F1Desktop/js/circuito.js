    ...
    cargarSVG(files)
    {
        ...
        if(tipoDelArchivo.match(tipoSVG))
        {
		    ...
		    lector.onload = function (evento) 
            {
                ...
                svg.attr("xmlns",atributoXMLNS);
                svg.attr("version",atributoVersion);

                var atributoStylePolyline = lineas[lineas.length-3].split("style=")[1].split("/>")[0].split('"')[1];
                ...
                var polyline = $("<polyline />")
                    .attr("points", puntosFormateados)
                    .attr("style", atributoStylePolyline);
